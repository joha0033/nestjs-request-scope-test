...
export class BenchmarkComparisonCommand extends CommandRunner {
  ...
  async run(passedParams: string[], options: BenchmarkOptions): Promise<void> {
    ...
    const { connections, duration, pipelining, query1, variables1, query2, variables2 } = options;
    ...
    console.log(`⚙️  Config: ${connections} connections, ${duration}s duration, ${pipelining} pipelining`);
    console.log(`🔍 Query 1: ${defaultQuery1.substring(0, 100)}${defaultQuery1.length > 100 ? '...' : ''}`);
    console.log(`🔍 Query 2: ${defaultQuery2.substring(0, 100)}${defaultQuery2.length > 100 ? '...' : ''}`);
    console.log('');
